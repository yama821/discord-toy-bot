...
class New(commands.Cog):
    ...
    async def new(
            ...
            pending_categories = (
                await session.execute(
                    select(Category).where(
                        ...
                        Category.state == SeminarState.PENDING
                    )
                )
            ).scalars().all()

        if not pending_categories:
            await ctx.respond("❌ 仮立て(PENDING)用のカテゴリーが設定されていません。管理者に `/setting` で設定を依頼してください。")
            return
        target_category: discord.CategoryChannel | None = None
        for cat_db in pending_categories:
            # Discord上からカテゴリーの実体を取得
            discord_category = ctx.guild.get_channel(cat_db.category_id)
            if not isinstance(discord_category, discord.CategoryChannel):
                continue
            
            # Discordの仕様上、1カテゴリーには最大50チャンネルまで
            if len(discord_category.channels) < 45:
                target_category = discord_category
                break

        # TODO: メッセージの修正, 新カテゴリーの追加
        if not target_category:
            await ctx.respond("⚠️ すべての仮立て用カテゴリーが満杯です！新しくカテゴリーを作成して設定してください。")
            return
        ...
        new_text_channel = await target_category.create_text_channel(name=seminar_name)
        embed = discord.Embed(
        ...
        # )
        # TODO: get_channel がうまくできない場合のエラーハンドリング
        role_setting_channel = ctx.guild.get_channel(
