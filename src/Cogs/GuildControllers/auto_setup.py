...
# ▼ 追加: 作成済みのEmbed生成関数をインポート
# （フォルダ名が UI の場合は from UI.settings_ui に読み替えてください）
from UI.settings_ui import get_settings_embed 
    ...
    async def _run_auto_setup(self, guild: discord.Guild):
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                system_channels = [c for c in guild.text_channels if "system" in c.name.lower()]
                ...
                # --- 絵文字の検出 ---
                interesting_emojis = [e for e in guild.emojis if e.name == "interesting"]
